...
class SuperCodeNode {
    constructor() {
        this.description = {
            ...
            inputs: [{ displayName: '', type: n8n_workflow_1.NodeConnectionType.Main }],
            outputs: [{ displayName: '', type: n8n_workflow_1.NodeConnectionType.Main }],
            credentials: [],
            properties: [
                {
                    displayName: 'Code',
...
`,
    ...
    async execute() {
        ...
        const executionMode = this.getNodeParameter('executionMode', 0, 'runOnceForAllItems');
        const code = this.getNodeParameter('code', 0);
        const timeout = this.getNodeParameter('timeout', 0, 30);
        if (!code.trim()) {
            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'No code provided. Please add your JavaScript/TypeScript code.');
        }
        // Create enhanced sandbox with lazy loading
        const createEnhancedSandbox = (items) => {
            ...
            const createLLMError = (type, libraryName, originalError, context) => {
                const errorCodes = {
                ...
                return new Error(`🤖 LLM-FRIENDLY ERROR [${code}]\n` +
                    `📍 Library: ${libraryName}\n` +
            ...
            const lazyLoad = (libraryName, requirePath, property) => {
            ...
            const sandbox = {
                ...
                console: {
                    log: (...args) => console.log('[SuperCode]', ...args),
                    error: (...args) => console.error('[SuperCode]', ...args),
                    warn: (...args) => console.warn('[SuperCode]', ...args),
                },
                utils: {
                    ...
                    retry: async (fn, options = { attempts: 3, delay: 1000 }) => {
                        let lastError;
                    ...
                    isLibraryLoaded: (libraryName) => !!libraryCache[libraryName],
        ...
        try {
            if (executionMode === 'runOnceForAllItems') {
				...
				`;
                const result = await (0, vm_1.runInContext)(wrappedCode, context, {
            ...
            throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Code execution failed: ${error.message}\\n\\nStack trace:\\n${error.stack}`);
        }
    }
}
exports.SuperCodeNode = SuperCodeNode;
