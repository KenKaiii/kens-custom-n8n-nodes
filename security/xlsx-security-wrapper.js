/**
 * Enhanced XLSX Security Wrapper (Compiled JavaScript)
 * 
...
const { ApplicationError } = require('n8n-workflow');
...
function validateForReDoS(data) {
 ...
 */
function createSecureXLSXWrapper(xlsxModule) {
	...
	return {
		...
		read: (data, opts) => {
			...
			try {
				...
				const result = xlsxModule.read(data, safeOpts);
		...
		readFile: (filename, opts) => {
			...
			try {
				// Call original method with sanitized inputs
				const result = xlsxModule.readFile(filename, safeOpts);
		...
		write: xlsxModule.write ? (workbook, opts) => {
			if (!workbook || typeof workbook !== 'object') {
			...
			try {
				return xlsxModule.write(workbook, safeOpts);
			} catch (error) {
				if (error instanceof ApplicationError) {
					throw error;
				}
		...
		writeFile: xlsxModule.writeFile ? (workbook, filename, opts) => {
			if (!workbook || typeof workbook !== 'object') {
			...
			try {
				return xlsxModule.writeFile(workbook, filename, safeOpts);
			} catch (error) {
				if (error instanceof ApplicationError) {
					throw error;
				}
...
module.exports = { createSecureXLSXWrapper };